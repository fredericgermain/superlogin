...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.logoutSession = function(session_id) {
    ...
    return userDB.query('auth/session', {key: session_id, include_docs: true})
      .then(function(results) {
        ...
        if (useSession) {
          [session_id].forEach(function(session) {
  ...
  function addUserDBs(newUser) {
    ...
    if (useSession) {
      newUser.dbPassword = util.URLSafeUUID();
      promises.push(
        couchAuthDB.put({
          _id: 'org.couchdb.user:' + newUser._id,
          type: 'user',
          name: newUser._id,
          password: newUser.dbPassword,
          roles: newUser.roles
        })
      );
    }
