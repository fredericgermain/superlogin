...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  var useSession = config.getItem('security.useSession');
  var useChannels = config.getItem('security.useChannels');
    ...
    return user.process()
      .then(function (result) {
        ...
        if(emailUsername) {
          ...
          newUser._id = 'superlogin:user:' + finalUsername;
        }
      ...
      .then(function(newUser) {
        return self.logActivity(newUser.id, 'signup', 'local', req, newUser);
      })
      .then(function(newUser) {
        return processTransformations(onCreateActions, newUser, 'local');
      })
      .then(function(finalNewUser) {
        var finalUser = finalNewUser;
        finalUser._id = 'superlogin:user:' + finalUser.id;
    ...
    return BPromise.resolve()
      ...
      .then(function(results) {
        if (results.rows.length > 0) {
          ...
          return BPromise.resolve(user.id);
        } else {
          newAccount = true;
          user = {};
          user[provider] = {};
      ...
      .then(function(finalUsername) {
        user.id = finalUsername;
        user._id = 'superlogin:user:' + finalUsername;
      ...
      .then(function(userDoc) {
        ...
        return self.logActivity(userDoc.id, action, provider, req, userDoc);
      })
      .then(function(userDoc) {
        if(newAccount) {
  ...
  this.linkSocial = function(user_id, provider, auth, profile, req) {
    ...
    return BPromise.resolve()
      ...
      .then(function() {
        ...
        delete user[provider].profile._raw;
        return self.logActivity(user.id, 'link', provider, req, user);
      })
      .then(function(userDoc) {
        return processTransformations(onLinkActions, userDoc, provider);
      })
  ...
  this.createSession = function(user_id, provider, req) {
    ...
    return userDB.get(user_id)
      .then(function(record) {
        user = record;
        return generateSession(user.id, user.roles, user.dbPassword);
      })
      .then(function(token) {
      ...
      .then(function() {
        ...
        return self.logActivity(user.id, 'login', provider, req, user);
      })
      .then(function(userDoc) {
      ...
      .then(function() {
        ...
        if(typeof user.personalDBs === 'object') {
          ...
          if(config.getItem('dbServer.publicURL')) {
            baseUrl = config.getItem('dbServer.publicURL');
          } else {
            baseUrl = config.getItem('dbServer.protocol') + config.getItem('dbServer.host') + '/';
          }
          var dbObj = url.parse(baseUrl);
          if (config.getItem('dbServer.couchbaseSyncGateway')) {
            newSession.RTDBUrl = dbObj.format() + config.getItem('dbServer.userDB');
          } else {
            var userDBs = {};
            var publicURL;
  ...
  this.handleFailedLogin = function(user, req) {
    ...
    return self.logActivity(user.id, 'failed login', 'local', req, user)
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
      })
  ...
  this.logActivity = function(user_id, action, provider, req, userDoc, saveDoc) {
    ...
    return promise
      .then(function(theUser) {
        ...
        if(saveDoc) {
          var finalUser = userDoc;
          finalUser._id = 'superlogin:user:' + finalUser.id;
          return userDB.put(finalUser)
            .then(function() {
              return BPromise.resolve(finalUser);
            });
        } else {
          return BPromise.resolve(userDoc);
        }
  ...
  this.resetPassword = function (form, req) {
    ...
    return passwordResetForm.validate()
      ...
      .then(function(userDoc) {
        ...
        return self.logActivity(user.id, 'reset password', 'local', req, user);
      })
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
  ...
  this.changePasswordSecure = function(user_id, form, req) {
    ...
    return changePasswordForm.validate()
      ...
      .then(function() {
        return self.changePassword(user.id, form.newPassword, user, req);
      }, function(err) {
  ...
  this.changePassword = function(user_id, newPassword, userDoc, req) {
    ...
    return promise
      ...
      .then(function(hash) {
        ...
        return self.logActivity(user.id, 'changed password', 'local', req, user);
      })
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
  ...
  this.forgotPassword = function(email, req) {
    ...
    return userDB.query('auth/email', {key: email, include_docs: true})
      .then(function(result) {
        ...
        return self.logActivity(user.id, 'forgot password', 'local', req, user);
      })
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
  ...
  this.verifyEmail = function(token, req) {
    ...
    return userDB.query('auth/verifyEmail', {key: token, include_docs: true})
      .then(function(result) {
        ...
        return self.logActivity(user.id, 'verified email', 'local', req, user);
      })
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
  ...
  this.changeEmail = function(user_id, newEmail, req) {
    ...
    return self.validateEmail(newEmail)
      ...
      .then(function() {
        ...
        return self.logActivity(user.id, 'changed email', req.user.provider, req, user);
      })
      .then(function(finalUser) {
        finalUser._id = 'superlogin:user:' + finalUser.id;
        return userDB.put(finalUser);
  ...
  this.addUserDB = function(user_id, dbName, type, designDocs, permissions) {
    ...
    return userDB.get(user_id)
      .then(function(result) {
        userDoc = result;
        if (useChannels) {
          userDoc.personalDBs[dbName] = { type: dbConfig.type };
          return;
        }
  ...
  this.logoutSession = function(session_id) {
    ...
    return userDB.query('auth/session', {key: session_id, include_docs: true})
      .then(function(results) {
        ...
        if (useSession) {
          [session_id].forEach(function(session_id) {
            promises.push(userDB.logout({
              session_id: session_id,
              headers: {
                cookie: `AuthSession=${session_id}`
              }
            }));
          });
        } else {
      ...
      .then(function(finalUser) {
        if(finalUser.session) {
          endSessions = Object.keys(finalUser.session).length;
        }
        emitter.emit('logout', finalUser.id);
    ...
    if(sessions.length) {
      ...
      if (useSession) {
        sessions.forEach(function(session) {
          const sessionId = (config.getItem('dbServer.couchbaseSyncGateway'))
            ? session
            : session.key;
          promises.push(userDB.logout({
            headers: {
  ...
  this.remove = function(user_id, destroyDBs) {
    ...
    return userDB.get(user_id)
      ...
      .then(function(userDoc) {
        ...
        if (useSession && couchAuthDB) {
          promises.push(
            couchAuthDB.remove(
              'org.couchdb.user:' + user_id
            )
          );
        }
        if (!useChannels) {
  ...
  this.removeExpiredKeys = useSession
    ? function () {}
    : dbAuth.removeExpiredKeys.bind(dbAuth);
  ...
  function generateSession(username, roles, userkey) {
    var getKey;
    if(config.getItem('dbServer.cloudant')) {
      getKey = require('./dbauth/cloudant').getAPIKey(userDB);
    } else if (useSession) {
      getKey = userDB.login(username, userkey)
        .then(function (response) {
          var token;
          if (response.statusCode === 200) { // couchDb style
            if (response.headers['set-cookie']) {
              var setCookie = response.headers['set-cookie'][0];
              var m = setCookie.match(/([^=]*)=([^;]*);/);
              token = m[2];
            } else {  // couchbase sync gateway
              token = response.body.session_id;
            }
          }
          if (token === undefined) {
            throw new Error('no token on couchDB reply');
          }
          return {
            key: token
          };
  ...
  function addUserDBs(newUser) {
    ...
    if (useSession) {
      var dbPassword = util.URLSafeUUID();
      if (couchAuthDB) {
        promises.push(
          couchAuthDB.put({
            _id: 'org.couchdb.user:' + newUser.id,
            type: 'user',
            name: newUser.id,
            password: dbPassword,
            roles: newUser.roles
    ...
    var processUserDBs = function(dbList, type) {
      dbList.forEach(function(userDBName) {
        // sync gateway mode
        if (useChannels) {
          newUser.personalDBs[userDBName] = { type };
          return;
        }
