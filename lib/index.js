...
module.exports = function (configData, passport, userDB, couchAuthDB) {
  ...
  if (config.getItem('dbServer.couchbaseSyncGateway')) {
    PouchDB.plugin(require('./dbauth/couchbase-syncgw'));
  } else if (config.getItem('security.useSession')) {
  ...
  if(!userDB && config.getItem('dbServer.userDB')) {
    ...
    if (config.getItem('dbServer.couchbaseSyncGateway')) {
      // patch view query with stale:false
      var oldQuery = userDB.query;
      userDB.query = function(id, opts) {
        return oldQuery.apply(userDB, [id, Object.assign(opts, {stale: false})]);
      }
    }
  }
  if(!couchAuthDB && config.getItem('dbServer.couchAuthDB') &&
     !(config.getItem('dbServer.cloudant') || config.getItem('dbServer.couchbaseSyncGateway'))) {
